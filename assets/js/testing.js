...
$(document).ready(function(){
	...
	/* gform validator items */
	
	var animation_speed = 350;

	function cloak_elem(elem) {
		jQuery(elem).css('visibility', 'hidden');	
	}

	function uncloak_elem(elem) {
		jQuery(elem).css('visibility', 'visible');	
	}

	function get_elem_error(elem) {
		return jQuery(elem).siblings('.validation_message');
	}

	function get_elem_list_item(elem) {
		return jQuery(elem).parent('div').parent('li');	
	}

	jQuery('input, textarea, select').focus(function() {
			cloak_elem(get_elem_error(this));
	});

	jQuery('input, textarea, select').blur(function() {
		if(get_elem_list_item(this).hasClass('gfield_error')) {
			uncloak_elem(get_elem_error(this));
		} else {
			var err = get_elem_error(this);
			err.slideUp(animation_speed);
		}
	});	
	...
	var validate_options = {
		...
		invalidHandler: function(form, validator) {
			...
			if(!jQuery('.gform_error').length) {
				jQuery('.gform_heading').after('<div class="gform_error">There was a slight problem with your request. Please correct the fields listed below.</div>');
			} else {
				jQuery('.gform_error').show();
			}
		//	scroll_to_error();
		},
	...
	function mixUp(){
		//if(document.getElementById('gloslinks')){ return false;}
		$("#stories dl").mixItUp({
			selectors : {
				target : 'div.mix'
				},
	...
	function formValidation(){
		$(".gform_wrapper form").validate(validate_options);
	...
	// Emails
	jQuery(".gfield_contains_required.email input").each(function() {
