...
/* Twitter CRON */
#TODO UNCOMMENT
add_action('my_hourly_event', 'fetch_tweets');
...
function fetch_tweets(){
  ...
  foreach($tweets->statuses as $tweet){
    ...
    $check = $wpdb->get_var(
        $wpdb->prepare(
        "SELECT tweetid FROM wnw_2011_tweets
        WHERE tweetid = %d
          ", $tweet->id
        )
    );
    ...
    if($check == null){
      echo $tweet->id . " | " . $tweet->user->name . " | " . $tweet->created_at;


      // item isnt in the db lets add it.
      $wpdb->insert(
        'wnw_2011_tweets',
        array(
          'tweetid' => $tweet->id,
          'postedby' => $tweet->user->name,
          'maintext' => $tweet->text,
...
function pull_tweets($n=4){
  global $wpdb;

  $query = "SELECT * FROM wnw_2011_tweets ORDER BY id DESC LIMIT 4";

  $tweets = $wpdb->get_results($query);

  foreach($tweets as $tweet){
    $output .= "<div class='col-sm-3'>";
    $output .= "<span class='user'>" . $tweet->postedby . "</span>\n";
    $output .= "<p class='tweet'>" .twitter_it($tweet->maintext) . "</p>";
    $output .= "</div>\n";
  }
  return $output;
}
